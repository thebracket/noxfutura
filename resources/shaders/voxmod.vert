...
void main() {
    ...
    vec3 rotated_position = i_rot == 0.0 ? base_position.xyz : (base_position * rotation).xyz;
    vec3 final_position = rotated_position + i_position + 0.5;

    v_normal = i_rot == 0 ? normal_lut[int(a_normal)] : (vec4(normal_lut[int(a_normal)], 0.0) * rotation).xyz;
