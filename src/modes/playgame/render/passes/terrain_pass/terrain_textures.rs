...
impl TerrainTextures {
    pub fn new() -> Self {
        ...
        let sampler = rc.device.create_sampler(
            &gpu::SamplerDescriptor {
        ...
        let bind_group = rc.device.create_bind_group(&gpu::BindGroupDescriptor{
            entries: &[
                gpu::BindGroupEntry {
                    binding: 0,
                    resource: gpu::BindingResource::TextureViewArray(
                        &tex_views
                    )
                },
                gpu::BindGroupEntry {
...
fn build_material_list(texture_set: &mut HashSet<String>) -> Vec<(usize, Option<String>, Option<String>, Option<String>, Option<String>)>
{
    ...
    rlock.materials.materials.iter().enumerate().for_each(|(i, m)| {
        let mut mat_map_tmp = (
            i,
            None,
            None,
...
fn build_texture_list(
    material_list_tmp: &Vec<(usize, Option<String>, Option<String>, Option<String>, Option<String>)>,
    ...
    material_list_tmp
        ...
        .map(|m| {
            (
                m.0,
                if let Some(t) = &m.1 {
                    texture_list.iter().enumerate().find(|(_i, tex)| **tex == *t).unwrap().0 as f32
                } else {
                    -1.0
                },
