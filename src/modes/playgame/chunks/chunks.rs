...
impl Chunks {
    ...
    pub fn on_camera_move(&mut self, camera_matrix: &Mat4, camera_position: &Position) {
        ...
        self.visible_chunks = self.chunks
            ...
            .enumerate()
            .filter(|(_i, c)| {
                self.frustrum.check_sphere(
                    &c.center_pos,
                    CHUNK_SIZE as f32 * 2.0
                )
            })
            .map(| (i, c) | (
                DistanceAlg::PythagorasSquared.distance3d(cp, Point3::new(c.center_pos.x as i32, c.center_pos.y as i32, c.center_pos.z as i32)),
                i
            ) )
            .collect();
        // Sort with nearest first to encourage z-buffer removal
        self.visible_chunks.sort_by(|a,b| a.partial_cmp(b).unwrap());
    }
